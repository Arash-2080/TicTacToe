...
class HardSinglePlayer extends StatefulWidget {
  const HardSinglePlayer({super.key});

  @override
  State<HardSinglePlayer> createState() => _HardSinglePlayer();
}

class _HardSinglePlayer extends State<HardSinglePlayer> {
  bool isTurnO = true;
  ...
  int findBestMove() {
    int bestScore = -1000;
    int bestMove = -1;
    int maxDepth = 2; // عمق حداکثر جستجو

    for (int i = 0; i < 9; i++) {
      if (xOroList[i] == '') {
        xOroList[i] = 'X';
        int moveScore = minimax(xOroList, 0, false, maxDepth);
        xOroList[i] = '';
  ...
  int minimax(List<String> board, int depth, bool isMaximizing, int maxDepth) {
    if (depth == maxDepth) {
      return 0; // متوقف کردن جستجو در عمق مشخص
    }
